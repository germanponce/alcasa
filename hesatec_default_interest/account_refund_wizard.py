...
class account_invoice(osv.osv):
    ...
    _columns = {
        ...
        'refund': fields.boolean('Se creo una devolucion'),
    }
...
class invoice_readjustment(osv.osv_memory):
    ...
    _columns = {
        ...
        'order_line'     : fields.one2many('account.invoice.simulation.line.refund', 'order_id', 'Líneas de Venta'),
        'detail_line'    : fields.one2many('account.invoice.simulation.detail.refund', 'order_id', 'Tabla de Amortización'),
            ...
            for invoice_act in account_invoice_obj.browse(cr, uid, [prueba.session_ids.id], context=context):
                ...
                #           invoice_vals_02['tax_amount'] = order.amount_tax
                    ...
                    raise osv.except_osv(
                                        _('Error !'),
                                        _('Para esta Factura ya se creo una Devolucion, Cliente %s'  % (invoice_act.partner_id.name)))
        return {
        ...
        for account in account_invoice_obj.browse(cr, uid, [session_ids], context=context):
            ...
            for line in account.invoice_line:
                xline = (0 ,0,{
                    ...
                    'xsubtotal': line.price_subtotal,
                    'xtotal': line.price_subtotal,
                    'xtax_amount': line.amount_tax,
...
class account_invoice_simulation_line_refund(osv.osv_memory):

    """ Account Invoice Simulation Line"""

    _name = 'account.invoice.simulation.line.refund'
    _rec_name = 'product_id'
    _columns = {
        'order_id'       : fields.many2one('invoice.refund.wizard', 'Invoice Reference', required=True),
        ...
        'factor': fields.float('Factor', digits=(14,2)) ,
        'proporcion': fields.float('Proporcion', digits=(14,2)), 
        'principal': fields.float('Principal', digits=(14,2)), 
        'interes': fields.float('Interes', digits=(14,2)), 
        'cuota': fields.float('Cuota', digits=(14,2)), 
        'prima': fields.float('Prima', digits=(14,2)), 
        'xpagos': fields.float('X Pagos', digits=(14,2)), 
        'dev_money' : fields.float(' Dev en dinero', digits=(14,2)), 
...
class account_invoice_simulation_detail_refund(osv.osv_memory):

    """ Account Invoice Simulation Line"""

    _name = 'account.invoice.simulation.detail.refund'
    _description = 'Sale Order Simulation Payments Detail _'

    _columns = {
        'order_id'       : fields.many2one('invoice.refund.wizard', 'Order Reference', required=True),
